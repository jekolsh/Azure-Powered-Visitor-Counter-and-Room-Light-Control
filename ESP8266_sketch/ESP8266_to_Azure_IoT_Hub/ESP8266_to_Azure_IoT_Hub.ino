...
int inCounter = 0;    
int outCounter = 0; 
int prevIrSensorValue = HIGH;
int prevIrSensorValue2 = HIGH;
...
static void sendTelemetry() {
  ...
  doc["irValue"] = irSensorValue; 
  doc["irValue2"] = irSensorValue2;
  doc["visitorCount"] = counter;

  char buffer[256];
  serializeJson(doc, buffer);

  // Send the JSON buffer
   mqtt_client.publish("devices/NodeMCU_ESP8266/messages/events/$.ct=application%2Fjson%3Bcharset%3Dutf-8", buffer, false);
...
// Read sensor values
  ...
  irSensorValue2 = digitalRead(IR_SENSOR_PIN2);

  // Check if sensor values have changed
  if (irSensorValue != prevIrSensorValue || irSensorValue2 != prevIrSensorValue2) {
    // Update previous sensor values
    prevIrSensorValue = irSensorValue;
    prevIrSensorValue2 = irSensorValue2;
    ...
    // Check if connected, reconnect if needed.
      ...
      inCounter += (irSensorValue == LOW);
      outCounter += (irSensorValue2 == LOW);
